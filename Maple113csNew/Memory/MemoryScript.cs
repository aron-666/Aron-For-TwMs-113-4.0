...
namespace ProcessTools.Memory
{
    public class ScriptObject
    {
        ...
        public ScriptObject(ScriptType type, IntPtr baseAddr, bool isread = true, int len = 0, Encoding encoding = null, params IntPtr[] offsets)
        {
            Type = type;
            Length = len;
        ...
        public bool DoScript(MemoryControl control, out object output)
        {
            output = DefaultObj;
            if (control == null) return false;
            try
            {
                var addr = control.ReadPtrs(BaseAddress, out bool b, PtrOffsets);
                if (!b) return false;
                if (!IsRead)
                {
                    //write
